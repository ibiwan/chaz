...
import {
  ...
  createSession,
  upsertSession,
} from '../dist/lib/sessions.js';

console.log("running server.mjs")
...
app.prepare().then(() => {
  ...
  wss.on('connection', (ws, req) => {
    ws.on('message', async (raw) => {
      ...
      if (msg.type === 'join') {
        console.log('[WS] Received join message:', msg);
        if (!msg.keyword || typeof msg.keyword !== 'string') {
          console.log('[WS] Missing keyword');
        ...
        console.log('[WS] Session after ensureSessionForKeyword:', session);
        ...
        console.log('[WS] Sending joined message:', { type: 'joined', session, role });
        sendWs(ws, { type: 'joined', session, role });
        console.log('[WS] Broadcasting session:', session);
