...
export function getSessionByKeyword(keyword: string): ChessSession | null {
  const row = selectByKeyword.get(keyword) as ChessSession;
  if (!row) return null;
  return {
    id: row.id,
...
export function getSessionByToken(token: string): ChessSession | null {
  const row = db
    .prepare('SELECT * FROM sessions WHERE player1_token = ? OR player2_token = ? LIMIT 1')
    .get(token, token) as ChessSession;
  if (!row) return null;
  return {
    id: row.id,
    player1_token: row.player1_token,
...
export function createSession(session: ChessSession) {
  insertSession.run({
    id: session.id,
...
export function getSession(id: string): ChessSession | null {
  const row = selectSession.get(id) as ChessSession;
  if (!row) return null;
  return {
    id: row.id,
    player1_token: row.player1_token,
...
export function upsertSession(session: ChessSession) {
  updateSession.run({
    id: session.id,
    player1_token: session.player1_token || null,
    player2_token: session.player2_token || null,
